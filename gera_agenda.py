...
def ler_dados(caminho_planilha='agenda.xlsx'):
    ...
    df = df.rename(columns={'Descrição': 'Descricao', 'Observação': 'Observacao', 
                            'Cobrança': 'Cobranca', 'Ordens Abertas': 'OrdensAbertas'})
    df = df.fillna('')
    
    stats = {
        'total': 0, 'CONCLUIDO': 0, 'EM_ANDAMENTO': 0, 'PENDENTE': 0, 'FALHA': 0,
        'por_tecnico': {}
    }
    
    # Padroniza o status para o CSS
    ...
    ordens = df['OrdensAbertas'].iloc[0] if not df.empty else '0'
...
def main():
    ...
    if dados:
        ...
        with open('index.html', 'w', encoding='utf-8') as f:
            f.write(html)
        print(f"🚀 Painel visual atualizado às {dados['data_atualizacao']}")
